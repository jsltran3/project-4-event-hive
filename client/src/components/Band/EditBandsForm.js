...
function EditBandForm({
  ...
  function handleChooseband(e) {
    ...
    let chosenConcertTicketBandsMatch = chosenConcertTicket.bands.find(
      (band) => band.name === bandMatch
    );

    let chosenBandIndex = chosenConcertTicket.bands
      .map((band) => band.name)
      .indexOf(bandMatch);
  ...
  const handleEdit = (e) => {
    ...
    fetch(`/concert_tickets/${concertTicketId}/bands/${bandId}`, {
      ...
      body: JSON.stringify({
        name: editBandFormData["name"],
        concert_ticket_id: concertTicketId,
      }),
    }).then((r) => {
      if (r.ok) {
        r.json().then((r) => {
  ...
  return (
    <div>
      ...
      <form>
        ...
        <br />
        <input
          onChange={handleEditbandChange}
          type="text"
          id="name"
          name="name"
          value={editBandFormData.name}
        />
        <br />
        <br />
